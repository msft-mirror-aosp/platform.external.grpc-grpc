...
"""
# We disable IOS support for the emu-dev, we don't want these dependencies.
...
def grpc_cc_library(
    ...
    if language.upper() == "C++":
        copts = copts + if_not_windows(["-std=c++17"]) + if_windows(["-std:c++17"])
...
def uses_unavailable_deps(deps):
    """Used by emu-dev to filter out package we do not make available."""
    unavailable = [
        "@com_github_cncf_xds",
...
def grpc_upb_proto_library(name, deps):
    if uses_unavailable_deps(deps):
        upb_proto_library(name = name, deps = ["//src/proto/grpc/lookup/v1:rls_config_proto_descriptor"])
        return
